...
const PortfolioBuySellStock = ({ portfolioId, isStockAdded, setIsStockAdded, type, stockId, stockName, currentPrice, setOpenDialog, status, setStatus }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // );
       const response = await axios.put(
        `http://abhiapi.trafficmanager.net:9000/portfolio/${type === "addMore" ? "buyStock" : "sellStock"}/${inputs.quantity}`,null,{params:{stockId:inputs.stockId,portfolioId:portfolioId}}
      );
